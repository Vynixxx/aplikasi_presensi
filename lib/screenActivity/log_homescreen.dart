...
import 'package:aplikasipresensi/model/logAktivitas.dart';
import 'package:aplikasipresensi/screenActivity/add_logscreen.dart';
import 'package:aplikasipresensi/screenActivity/update_logscreen.dart';
...
class _ActivityLogsScreenState extends State<LogsActivityScreen> {
  late Future<List<LogAktivitas>> futureLogs;
  ...
  Future<List<LogAktivitas>> fetchActivityLogs() async {
    ...
    if (response.statusCode == 200) {
      ...
      return jsonResponse.map((log) => LogAktivitas.fromJson(log)).toList();
    } else {
  ...
  void _editLog(LogAktivitas log) {
    Navigator.push(
      context,
      MaterialPageRoute(
